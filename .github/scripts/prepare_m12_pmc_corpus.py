...
def _https_url(value: str) -> str:
    ...
    if parsed.scheme == "ftp" and parsed.hostname == "ftp.ncbi.nlm.nih.gov":
        return urllib.parse.urlunparse(("https", parsed.netloc, parsed.path, "", "", ""))
    if parsed.scheme == "https" and parsed.hostname in {"ftp.ncbi.nlm.nih.gov", "pmc.ncbi.nlm.nih.gov"}:
        return value
...
def _candidate_pages() -> list[Candidate]:
    ...
    while len(candidates) < 5000:
        ...
        for record in root.findall(".//record"):
            ...
            if license_type not in ALLOWED_LICENSES:
                continue
            package = next(
                (link.get("href") for link in record.findall("link") if link.get("format") == "tgz"),
                None,
            )
    ...
    return sorted({item.pmcid: item for item in candidates}.values(), key=lambda item: int(item.pmcid[3:]))
...
def _write_manifest(runtime: Path, selected: list[Candidate]) -> Path:
            ...
            writer.writerow(
                {
                    ...
                    "inclusion_reason": "PMC OA dataset record with machine-readable reuse license.",
                    "source_type": "paper",
    ...
    corpus = {
        ...
        "description": "Ephemeral controlled corpus selected from the official PMC OA dataset service.",
        "scientific_domain": "biomedical science",
        "research_question": {
            "question_id": "q_m12_ingestion_rehearsal",
            "text": "Can the current ingestion system process a controlled real 100-paper corpus reproducibly?",
        },
...
def main() -> None:
    ...
    for candidate in _candidate_pages():
        ...
        try:
            ...
            failures.append(f"{candidate.pmcid}: {type(exc).__name__}")
            time.sleep(0.05)
    if len(selected) != args.count:
        raise RuntimeError(f"selected {len(selected)} usable PDFs; required {args.count}")
