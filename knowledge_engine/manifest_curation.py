...
def export_manifest_curation_draft(
    ...
    if not isinstance(adjudication_rows, list) or worksheet.get("candidate_count") != len(adjudication_rows):
    ...
    for row in adjudication_rows:
        ...
        if pmid in accepted:
            raise ManifestCurationError("Adjudication worksheet contains a duplicate accepted PMID.")
        accepted[pmid] = row
    ...
    for item in receipt_rows:
        ...
        if adjudication is None:
            raise ManifestCurationError("Receipt references a PMID without an accepted adjudication.")
        ...
        if (
            ...
            raise ManifestCurationError("Adjudicated evidence does not match the acquisition receipt.")
        if (
