"""Standalone CLI for exporting Europe PMC adjudications to acquisition approvals."""
...
app = typer.Typer(help="Export accepted M34 Europe PMC adjudications to acquisition approvals.")

WorksheetOption = Annotated[
    Path,
    typer.Option("--worksheet", help="Completed Europe PMC adjudication worksheet JSON path."),
]
...
def export_command(
    ...
    try:
        batch = export_europepmc_reviewed_approvals(worksheet, selection_limit=limit)
    except EuropePmcReviewedApprovalError as exc:
        typer.echo(f"Europe PMC adjudicated approval export failed: {exc}", err=True)
    ...
    typer.echo(
        ...
        "Europe PMC records for automated acquisition. Held and rejected records were excluded."
    )
