...
- **OpenAlex retry/rate-limit tracking (issue #433, item 2)**:
  ...
  and provider-unavailable/connection-level errors) with a bounded
  exponential backoff (3 attempts by default) instead of making exactly one
  attempt -- never retrying a non-transient 4xx client error, a 404 (still
  `EMPTY`), or a malformed/oversized response. This applies the same
  `max_attempts`/`retry_backoff_seconds`/`sleep` constructor parameters and
  retry-loop structure `semantic_scholar_provider.py` established; no
  further shared-infrastructure change was needed since `ProviderStatus`'s
  outcome-derived `rate_limited_observed` and the ledger/coverage-report
  plumbing were already generalized across every adapter by the prior
  Semantic Scholar slice. `arxiv_provider.py` and
  `pubmed_federated_adapter.py`/`crossref_federated_adapter.py` remain
  single-attempt and are the next natural slices.
...
- **Bounded-concurrency Europe PMC OA acquisition downloads (issue #433,
  ...
  thread-pool pattern used for PMC OA acquisition -- a new keyword-only
  `max_concurrent_downloads` constructor parameter (default 4), a sliding
  submission window so at most `max_concurrent_downloads` downloads are ever
  in flight or holding a completed response body in memory at once, and
  results are still consumed back in deterministic plan order so receipt
  ordering, atomic rollback, and per-approval failure-ordinal error messages
  are unchanged -- only multi-PDF batch wall-clock time improves. CORE and
...
- **Executable PMC acquisition and import provenance (CORE-GQR-4)**:
  ...
  parses and persists or reuses each Paper, and records one immutable
  `ImportRun` plus per-candidate `ImportItem` lineage. The receipt now
  carries the resulting import-run/item IDs alongside search-run, research
  question, candidate, PMID/PMCID, file-hash, and Paper provenance. A failed
  persistence or receipt write rolls back the batch and acquired PDFs.
