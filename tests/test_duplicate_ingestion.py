...
def _counts(database: object) -> tuple[int, int, int]:
...
def _seed_paper(database: object, parsed: ParsedPaper) -> int:
    with database.session() as session:  # type: ignore[attr-defined]
...
def test_exact_hash_duplicate_skips_without_paper_text_or_fts_write(tmp_path: Path) -> None:
    ...
    assert json.loads(item.duplicate_evidence_json or "{}")["reason_code"] == "matching_content_hash"
...
def test_doi_hash_conflict_requires_review_without_persistence_write(tmp_path: Path) -> None:
    ...
    assert evidence["reason_code"] == "matching_doi_conflicting_content_hash"
    assert _counts(database) == before
