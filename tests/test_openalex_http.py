...
from knowledge_engine.openalex_http import (
...
@pytest.mark.parametrize(
    "url",
    [
        "http://api.openalex.org/works?search=test",
        "https://example.com/works?search=test",
        "https://user:pass@api.openalex.org/works?search=test",
        "https://api.openalex.org:444/works?search=test",
    ],
)
def test_transport_rejects_unsafe_or_unsupported_urls(url: str) -> None:
    transport = UrllibOpenAlexTransport()
