...
The active ruleset is `m14-candidate-adjudication-v9`.

- `metadata_only` records are rejected for the PMC OA acquisition path with `NO_VERIFIED_REUSABLE_FULL_TEXT`.
- A title starting with a non-primary-content prefix (Correction:, Corrigendum:, Erratum:, Retraction:, Retracted:, Publisher Correction:, Author Correction:) fails scientific evidence outright -- a correction/erratum/retraction notice for an original article is not itself a scientific paper, systematic review, meta-analysis, or clinical research report, and typically only amends a figure, table, or author list; it inherits the original article's title, so it would otherwise pass disease/intervention keyword matching whenever the original did. (v8.)
- Scientific evidence passes when the combined PubMed title and abstract contain both a declared metabolic-disease term and a declared treatment or therapeutic term, anywhere in the combined text. **v8 briefly required both terms to share a single sentence** (to reject a case report Codex flagged, where a disease term appeared only as an incidental patient comorbidity, unrelated to the paper's actual topic, in a different sentence from a generic intervention word). Re-adjudicating a real 250-candidate discovery batch under that rule dropped 33 of 74 previously accepted records (44%) -- almost all legitimately on-topic papers whose structured or narrative abstracts routinely name the disease in one sentence and describe treatment in another. **v9 reverts the same-sentence requirement**: the cost of losing that much legitimate corpus content is judged materially worse than occasionally accepting a rare incidental-comorbidity false positive, and this deterministic, non-ML ruleset has no subtler way to tell the two cases apart.
...
- A title containing a pediatric-population term (pediatric, paediatric, child, infant, neonat, adolescent, youth) with no adult-population term also present fails scientific evidence -- `exclusion_criteria.md` requires excluding sources "limited to" pediatric populations, not merely mentioning one, so a title also naming an adult population (e.g. "...in adolescents and adults") is mixed-age evidence and is not held. Checked against the title only, not the abstract, since an adult study's abstract can mention pediatric research as background without the study itself being pediatric. (v7; a v6 gap let three pediatric-titled records be accepted before a matching age/population term happened to also satisfy every other rule.)
