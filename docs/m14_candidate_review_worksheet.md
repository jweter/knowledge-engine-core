...
The active ruleset is `m14-candidate-adjudication-v8`.
...
- Scientific evidence passes only when the combined PubMed title and abstract contain both a declared metabolic-disease term and a declared treatment or therapeutic term, **and** at least one sentence of the combined evidence contains both terms together -- a disease term appearing only as an incidental patient comorbidity in one sentence, with an intervention term appearing only in a different sentence describing treatment for the paper's actual (unrelated) topic, does not satisfy scientific scope. (v8; a v7 gap let an unrelated case report -- e.g. persistent hiccups, with type 2 diabetes listed only as one of several patient comorbidities -- pass whenever any sentence anywhere in the abstract happened to use a generic word like "therapy" or "treatment".)
