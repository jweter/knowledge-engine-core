...
place. M33 added the second `VectorIndex` backend, `QdrantVectorIndex`,
targeting an operator-run Qdrant server; CLI wiring for it is deliberately
not built yet (no operator need for it has appeared).
...
- Support local FAISS and server-backed Qdrant. (Both implemented -- FAISS
  M30, Qdrant M33; Qdrant is not yet wired into the CLI commands.)
