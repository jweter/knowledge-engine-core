...
owner's decision) reproducible -- see `docs/m27_corpus_library.md`. The
snapshot itself is a local, regenerable cache rather than something git
commits: past ~605 papers it exceeds GitHub's 100MB single-file limit, and
since it rebuilds deterministically from the committed `sources.csv` plus
the Drive-archived PDFs, there is no durability reason to fight that limit.
Actually growing the corpus to the owner's "at least a couple thousand
papers" target remains ongoing operational work using this tooling plus
the existing M14 pipeline, not itself scheduled as a numbered milestone.
