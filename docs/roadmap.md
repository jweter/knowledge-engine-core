...
- `docs/ai_layer_architecture.md` -- the AI repository's Retrieval/
  ...
  canonical, team-authored version lives in `knowledge-engine-ai`'s
  own `docs/roadmap/future_ai_orchestration_plan.md` (this repo's copy
  of the same filename is a short pointer, not a duplicate): four
  durable contracts -- `ResearchPlan`/`ResearchSession`/`ResearchTask`/
  `ResearchEvent` -- executed by pluggable engines, an
  eleven-milestone `AI-O1`-`AI-O11` build order, and 16 named design
  risks with mitigations. Gated by evidence-base
