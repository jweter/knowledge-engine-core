...
- **Status:** open (mitigated) — the retry/backoff and diagnosability gap is fixed, unit-tested, and confirmed live at small scale (150 candidates); the exact original cause of the specific 3,250-candidate failure remains unconfirmed, and a full-scale live run has not yet re-exercised the fix.
