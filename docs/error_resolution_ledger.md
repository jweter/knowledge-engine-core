...
## 2026-07-21 — M14 PMC OA discovery and acquisition migrated to NCBI Cloud Service

- **Area:** runtime / reliability / external provider
- **First failing command:** N/A — proactive migration, not a reactive fix. NCBI announced (Feb/Apr 2026) that both the PMC OA Web Service API (`oa.fcgi`, used by discovery) and the PMC FTP Service (used by acquisition, including the `/pub/pmc/deprecated/` bridge from the entry above) are removed entirely in August 2026.
- **Symptom:** N/A at time of migration — this closes [issue #79](https://github.com/jweter/knowledge-engine-core/issues/79) before it becomes a live failure, not after.
- **Affected files:** `knowledge_engine/ncbi_http.py`, `knowledge_engine/pubmed_discovery.py`, `knowledge_engine/pmc_acquisition.py`, `knowledge_engine/candidate_review.py`, `knowledge_engine/reviewed_approval.py`, `.github/workflows/m14-mass-discovery.yml`, plus all affected tests and M14 docs. Full technical writeup: `docs/architecture/adr/0004-migrate-pmc-oa-acquisition-to-cloud-service.md`.
- **Root cause:** N/A — see ADR 0004's Context section for the confirmed NCBI announcements and live-verified API contract.
- **Fix:** See ADR 0004. Summary: both discovery's OA lookup and acquisition's PDF download now use NCBI's public `pmc-oa-opendata` S3 bucket (plain unsigned HTTPS, no new dependency), gated on the bucket's `is_pmc_openaccess` field rather than "did `oa.fcgi` return a record." Three independently-drifting hardcoded copies of the expected PDF host (found by auditing the whole repo) were consolidated into one `PMC_CLOUD_PDF_HOST` constant. `ADJUDICATION_RULES_VERSION` bumped `v3` → `v4`.
- **Validation:** Local: `poetry run ruff check .` / `ruff format --check .` / `mypy knowledge_engine tests` all clean; `poetry run pytest` — 460 collected, 0 failed (up from 456, reflecting new test coverage). Live, in two stages against real NCBI infrastructure:
  1. Bounded smoke test (`workflow_dispatch` run `29873299611`, commit `3ceeb97`, `limit=150`): `Run bounded mass discovery` succeeded — `Wrote 150 unique candidates across 2 page(s); 150 PMC OA verified; 0 duplicate PMID(s) removed.` Stopped as expected at `Select exactly 500 accepted records` (150 candidates cannot supply 500 accepted rows) — not a bug.
  2. Full original scale (`workflow_dispatch` run `29873704311`, commit `3ceeb97`, `limit=3250`, same query as the original failures): completed with `conclusion: success` in 1h12m (22:25:16–23:37:08 UTC; slower than the pre-migration ~28min baseline because the new OA lookup is 2 requests per candidate instead of 1). Every step passed, including the two never previously exercised against this replacement source: `Export manifest curation draft` → `Exported 500 manifest curation rows.` and `Validate corpus readiness` → `Corpus ready: 500 manifest rows, 500 receipts, 500 PDFs.` Reconciliation summary: `candidate_count=3250 fetched_page_count=33 pmcid_resolved=3250 (rate=1.000000) oa_verified=3243 (rate=0.997846) accepted=1866 rejected=7 held=1377 selected_count=500 acquired_count=500 total_pdf_bytes=1246312495 exhausted=False`.
- **Prevention / fast path:** If PMC OA discovery or acquisition fails again, check `https://pmc.ncbi.nlm.nih.gov/tools/pmcaws/` first for further NCBI Cloud Service structure changes before assuming a code regression. When adding any new hardcoded expected-host string for an NCBI resource, import `PMC_CLOUD_PDF_HOST` from `knowledge_engine.ncbi_http` instead of a new literal — this migration found three independent copies that had already drifted out of sync with each other.
- **Status:** resolved

## 2026-07-22 — License adjudication accepted restricted CC variants via prefix matching

- **Area:** runtime / correctness / legal compliance
- **First failing command:** N/A — discovered during M14 controlled-rehearsal preparation (issue #21) while inspecting a live discovery batch's license distribution before staging it for import, not from a failing test or CI run.
- **Symptom:** `candidate_review.py`'s `_license_result()` tested `normalized.startswith(("CC BY", "CC0"))` to decide whether a reported license was reusable enough for acquisition and downstream derivative-work extraction. Python's `str.startswith()` performs a literal character-prefix match, not a semantic license match, so `"CC BY-NC".startswith("CC BY")`, `"CC BY-NC-ND".startswith("CC BY")`, and `"CC BY-NC-SA".startswith("CC BY")` all evaluate `True` — every restricted CC BY variant was silently accepted as if it were the fully-reusable `CC BY` license. Measured against the most recent live 500-paper M14 batch (`approvals-500.json`, produced by workflow run `29873704311`): only 312 rows (62.4%) were genuinely `CC BY`; 188 rows (37.6%) were restricted (`CC BY-NC`: 95, `CC BY-NC-ND`: 92, `CC BY-NC-SA`: 1) and should have been held instead of accepted. This affected every M14 batch produced since the adjudication rules existed, including the batch cited as evidence in the PMC Cloud Service migration PR (#83).
- **Affected files:** `knowledge_engine/candidate_review.py`, `tests/test_candidate_review.py`, `docs/history/milestones/m14_candidate_review_worksheet.md`, `docs/history/milestones/m14_pmc_oa_acquisition.md`
- **Root cause:** Confirmed by direct inspection of `_license_result()` and reproduced with a standalone Python check (`"CC BY-NC".startswith("CC BY")` → `True`). `CC BY-NC` (no commercial use), `CC BY-NC-ND` (no commercial use, no derivatives), and `CC BY-NC-SA` (no commercial use, share-alike/copyleft) are legally distinct, more restrictive licenses than `CC BY`, not variants of it. `CC BY-ND` and `CC BY-SA` share the same false-positive risk though neither appeared in the sampled batch. The ND (no-derivatives) restriction is a direct conflict with this project's Phase 2 goal of extracting and redistributing derived claim/evidence records.
- **Fix:** Replaced the prefix tuple with `_ALLOWED_LICENSE_PATTERN = re.compile(r"^(?:CC BY|CC0)(?: [0-9][0-9.]*)?$")`, matched with `.fullmatch()` instead of `.startswith()`. This accepts only exactly `CC BY` or `CC0`, optionally followed by a version number (e.g. `CC BY 4.0`, `CC0 1.0`), and rejects any suffix beyond a version number — including every `-NC`, `-ND`, and `-SA` variant. Bumped `ADJUDICATION_RULES_VERSION` from `m14-candidate-adjudication-v4` to `m14-candidate-adjudication-v5` and updated the version references in both M14 docs to match.
- **Validation:** `poetry run pytest tests/test_candidate_review.py -q` — 20 passed, including two new parametrized tests: `test_oa_candidate_with_restricted_cc_variant_is_held` (`CC BY-NC`, `CC BY-NC-ND`, `CC BY-NC-SA`, `CC BY-SA`, `CC BY-ND` — all correctly held with `unsupported_license_basis`) and `test_oa_candidate_with_unrestricted_cc_variant_passes` (`CC BY`, `CC BY 4.0`, `CC0`, `CC0 1.0` — all correctly pass). Full suite: `poetry run ruff check .` clean, `poetry run ruff format --check .` clean, `poetry run mypy knowledge_engine tests` — success, no issues found in 113 source files, `poetry run pytest` — 469 passed.
- **Prevention / fast path:** Never use `str.startswith()` (or any prefix/substring check) to compare license identifiers or other short controlled-vocabulary strings where one valid value can be a literal character-prefix of a different, incompatible value. Use an explicit allowlist, exact match, or an anchored regex (`fullmatch`) instead. If a new CC variant or license string needs evaluation, check the full identifier including every suffix before assuming a shared prefix implies equivalent terms.
- **Status:** resolved

## 2026-07-22 — M14 manifest curation left import-blocking fields blank

- **Area:** runtime / correctness / import pipeline
- **First failing command:** N/A — discovered while preparing the M14 controlled rehearsal (issue #21) after downloading a fresh, license-compliant 500-paper batch produced by the fix above, before attempting `ke corpus-import` on it.
- **Symptom:** Every row `manifest_curation.py` exports would fail `corpus/validation.py`'s import checks in three independent ways: (1) `expected_content_hash` was written as bare 64-character hex, but `HASH_RE` requires a `sha256:` prefix; (2) `license_url` was left blank on every row, but `_validate_conditional_fields` treats it as a required import blocker whenever `usage_status == "approved_open_access"` (true for every M14 row); (3) `access_date` was left blank, but the same validator requires it whenever `inclusion_status == "included"` (also true for every M14 row). Confirmed directly against the real downloaded batch: `manifest-draft.csv` had blank `license_url` and `access_date` on all 500 rows and a 64-character unprefixed hash. `docs/history/milestones/m14_manifest_curation_draft.md` explicitly documented both blank fields as intentional ("deferred optional metadata... does not create a manual prerequisite for the M14 working version"), directly contradicting the general corpus validator's requirements.
- **Affected files:** `knowledge_engine/manifest_curation.py`, `knowledge_engine/candidate_review.py`, `tests/test_manifest_curation.py`, `docs/history/milestones/m14_manifest_curation_draft.md`
- **Root cause:** The M14 manifest curation exporter was designed and documented as if `license_url` and `access_date` had no deterministic automated source, so it always left them blank. Investigation showed `access_date` is derivable from the adjudication's own `adjudicated_at` timestamp (already required and validated), and `license_url` is derivable from `license_type` because, after the license-matching fix above, every accepted row's license is guaranteed to be exactly `CC BY` or `CC0` (optionally versioned) — a fixed, enumerable set with well-known canonical Creative Commons deed URLs. Neither field needed to stay blank; the exporter just never computed them. The `expected_content_hash` bare-hex bug was a simple missing-prefix oversight: the value already comes straight from the acquisition receipt's SHA-256 digest.
- **Fix:** Added `license_deed_url()` to `candidate_review.py` (the module that already owns `_ALLOWED_LICENSE_PATTERN`, the single source of truth for which licenses are allowed) mapping `CC BY[ <version>]` to `https://creativecommons.org/licenses/by/<version>/` and `CC0[ <version>]` to `https://creativecommons.org/publicdomain/zero/<version>/`, defaulting to the current version (`4.0` / `1.0`) when unversioned. `manifest_curation.py` now imports this instead of re-deriving license semantics independently — deliberately avoiding a second drifting copy of the same logic (the exact failure mode fixed for `PMC_CLOUD_PDF_HOST` during the PMC Cloud Service migration). `access_date` is now the date portion of the accepted adjudication's `adjudicated_at`. `expected_content_hash` is now written as `f"sha256:{sha256}"`. Updated `docs/history/milestones/m14_manifest_curation_draft.md` to move both fields out of "Deferred optional metadata" and document their deterministic derivation.
- **Validation:** `poetry run pytest tests/test_manifest_curation.py tests/test_candidate_review.py -q` — 30 passed, including two new tests (`test_export_manifest_curation_draft_maps_cc0_license_to_deed_url`, `test_unsupported_license_evidence_has_no_canonical_deed_url`) and an extended assertion on the existing populated-fields test confirming the `sha256:` prefix, `access_date`, and `license_url`. Full suite: `poetry run ruff check .` clean, `poetry run ruff format --check .` clean, `poetry run mypy knowledge_engine tests` — no issues in 113 source files, `poetry run pytest` — 471 passed. Live-data validation: regenerated `manifest-draft.csv` locally from the real worksheet and receipt downloaded from the merged fix's M14 Mass Discovery run (`29888601770`) — all 500 rows now have non-blank `access_date` and `license_url`, and a correctly prefixed hash. `poetry run python -m knowledge_engine.corpus_readiness_cli validate` against the regenerated manifest, the real receipt, and the real 500 downloaded PDFs reported `Corpus ready: 500 manifest rows, 500 receipts, 500 PDFs.`
- **Prevention / fast path:** Before declaring any field "deferred" or intentionally blank in an M14 pipeline doc, check whether it is actually derivable from evidence the pipeline already possesses (a timestamp already on the record, a value already constrained to a small enumerable set) before accepting a validator contradiction as permanent. When a value's legality is already validated against a pattern or allowlist elsewhere in the codebase (like `_ALLOWED_LICENSE_PATTERN`), derive dependent values from that same source instead of hardcoding a second copy.
- **Status:** resolved

## 2026-07-22 — License version pattern accepted non-existent Creative Commons versions

- **Area:** runtime / correctness / legal compliance
- **First failing command:** N/A — found by a Codex automated review comment on PR #85 (`knowledge_engine/candidate_review.py:16`), before the PR merged.
- **Symptom:** `_ALLOWED_LICENSE_PATTERN`'s version suffix, `(?: [0-9][0-9.]*)?`, matched any run of digits and dots after `CC BY` or `CC0` rather than a real Creative Commons version. Confirmed directly: `CC0 2.0` (CC0 only ever had a 1.0 release), `CC BY 4..0`, `CC BY 4.0.`, and `CC BY 99.0` all matched. Since this same pattern gates both adjudication acceptance (`_license_result`) and the license-to-deed-URL mapping added for manifest curation (`license_deed_url`, in the PR under review), malformed upstream license evidence that happened to match the loose pattern would be accepted as `passed`, and `license_deed_url` would emit a URL pointing at a Creative Commons deed that does not exist (e.g. `https://creativecommons.org/publicdomain/zero/2.0/`) - while `corpus/validation.py` only checks that `license_url` is non-empty, so the bogus URL would satisfy the import validator and the row would become an importable approved-open-access record.
- **Affected files:** `knowledge_engine/candidate_review.py`, `tests/test_candidate_review.py`, `docs/history/milestones/m14_candidate_review_worksheet.md`, `docs/history/milestones/m14_pmc_oa_acquisition.md`
- **Root cause:** The version suffix was written as a general "looks like a version number" character class instead of an explicit allowlist of the versions Creative Commons has actually published (CC BY: 1.0, 2.0, 2.5, 3.0, 4.0; CC0: 1.0 only). No real PMC data in this project has ever supplied a versioned license string (the confirmed live batch is 500/500 bare `CC BY`), so this was a latent defect, not one observed in production data - but it was reachable by any malformed or adversarial upstream value that happened to match `[0-9.]*`.
- **Fix:** Replaced `(?: [0-9][0-9.]*)?` with an explicit alternation of real published versions: `(?: (?:1\.0|2\.0|2\.5|3\.0|4\.0))?` for CC BY and `(?: 1\.0)?` for CC0. Bumped `ADJUDICATION_RULES_VERSION` from `v5` to `v6` and updated both M14 docs to match. No change was needed to `license_deed_url` itself - it already only receives values that already passed this pattern, so tightening the pattern alone closes the gap.
- **Validation:** `poetry run pytest tests/test_candidate_review.py tests/test_manifest_curation.py -q` — 51 passed, including new tests for every real CC BY version (1.0-4.0) still passing, five malformed-but-previously-accepted strings (`CC0 2.0`, `CC BY 4..0`, `CC BY 4.0.`, `CC BY 99.0`, `CC BY 5.0`) now correctly held, and direct unit tests of `license_deed_url` covering all valid versions plus its `ValueError` on unsupported input. Full suite: `poetry run ruff check .` clean, `poetry run ruff format --check .` clean, `poetry run mypy knowledge_engine tests` — no issues in 113 source files, `poetry run pytest` — 494 passed. Regenerated `manifest-draft.csv` from the same real 500-paper batch used to validate the fix above; unaffected, since every real license string is bare `CC BY`.
- **Prevention / fast path:** When validating a version number that belongs to a small, fixed, externally-published set (a license version, a schema version, a protocol version), enumerate the real values explicitly rather than matching a general numeric-looking shape - a shape-only pattern accepts values that were never published and have no real resource behind them.
- **Status:** resolved

## 2026-08-08 — PDF corpus recovery rejected every PMC-hosted resolution

- **Area:** runtime / reliability / external provider
- **First failing command:** `python3 tools/reacquire_corpus_pdfs.py --database data/knowledge_engine.sqlite3 --target-dir <empty dir> --dry-run --limit 5`
- **Symptom:** `tools/reacquire_corpus_pdfs.py` (built for M14-adjacent corpus recovery, per its own docstring never yet run) failed 3 of the first 5 real papers tested against this environment's live 951-paper corpus, each with `status: resolution_failed` and `note: "PMC metadata returned a non-allowlisted PDF URL."` Direct reproduction: fetching `https://pmc-oa-opendata.s3.amazonaws.com/metadata/PMC9515581.1.json` live returned `"pdf_url": "s3://pmc-oa-opendata/PMC9515581.1/PMC9515581.1.pdf?md5=..."` — an `s3://` URI, not an `https://` URL. `_pmc_resolution()`'s allowlist check (`parsed.scheme != "https" or parsed.hostname != PMC_CLOUD_HOST`) unconditionally rejected this shape, so every PMC-hosted resolution failed regardless of the paper's actual open-access status. A full dry run across all 960 real corpus rows confirmed this was systemic, not sporadic: before the fix, every row that should have routed through the PMC Article Datasets Cloud Service (the tool's preferred path) instead fell through to `resolution_failed` or `open_access_source_unresolved`.
- **Affected files:** `tools/reacquire_corpus_pdfs.py` (no test file existed for this tool before this entry — zero coverage let the bug ship silently since the tool's introduction).
- **Root cause:** PMC's Article Datasets Cloud Service metadata JSON records `pdf_url`/`text_url`/`xml_url` as `s3://<bucket>/<key>` URIs, not HTTPS URLs. The tool's allowlist check assumed an HTTPS URL matching the same virtual-hosted-style host (`pmc-oa-opendata.s3.amazonaws.com`) already used for the listing and metadata requests, but never converted the `s3://` scheme before checking it.
- **Fix:** Added `_pmc_cloud_pdf_url()`, which parses the `s3://` URI, validates the bucket is exactly `pmc-oa-opendata` (rejecting any other scheme or bucket rather than silently fetching it), and reconstructs the HTTPS virtual-hosted-style URL (`https://pmc-oa-opendata.s3.amazonaws.com/<key>?<query>`), preserving the `md5` query string. `_pmc_resolution()` now calls this instead of validating the raw metadata value directly.
- **Validation:** Added `tests/tools/test_reacquire_corpus_pdfs.py` (previously nonexistent) covering `_pmc_cloud_pdf_url` (valid conversion with and without a query string, rejection of a non-`s3` scheme, rejection of a non-`pmc-oa-opendata` bucket, rejection of an empty key), `normalize_doi`, `safe_filename`, and `sha256_file` — 9 tests, all passing. Live re-verification against the real local corpus: a dry run across all 960 database rows went from 3/5 sampled `resolution_failed` before the fix to `already_present_verified: 951, dry_run_resolved: 9` (0 failures) after. A real (non-dry-run) recovery of the 9 papers with no local PDF file (IDs 944-949, 956-958, found via `build_corpus_recovery_manifest.py`, itself run live for the first time here) downloaded and hash-verified all 9 against their historical SHA-256 (`reacquired_verified` for all 9). `build_corpus_recovery_manifest.py` re-run afterward confirmed `"pending": 0` across the full 960-row corpus. `poetry run ruff check .` / `ruff format --check .` clean; `poetry run pytest` full suite green.
- **Prevention / fast path:** When consuming a new field from an external metadata API for the first time, verify its actual runtime value against a live call before writing a URL/scheme allowlist check against it — do not assume a sibling field's format (the listing/metadata endpoints here really do return HTTPS) extends to every field in the same payload. This tool had never been run against real data before this entry; any recovery/acquisition tool with zero live-network test coverage should be treated as unverified regardless of how carefully it reads.
- **Status:** resolved
