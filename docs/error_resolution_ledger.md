...
- **Status:** open (mitigated) — fixed, unit-tested, and confirmed via a local synthetic end-to-end integration run reproducing the real over-accept/select shape; not yet confirmed against a live full-scale M14 workflow run with real discovery and acquisition output. Will move to resolved once that live run passes.
